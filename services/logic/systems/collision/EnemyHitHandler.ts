        ...
        for (const e of neighbors) {
            ...
            if (dist < proj.radius + e.radius) {
                ...
                if (BattleFormulas.shouldTriggerRedEnvelope(p.items.includes('红包'))) {
                    p.gold += 2; engine.state.score += 2; 
                    engine.state.waveStats.goldEarned += 2; engine.state.waveStats.bonusGold += 2;
                }
                ...
                if (e.config.behavior !== 'boss') { 
                    ...
                    const force = isSweeperCheck ? knockbackValue * 2.5 : knockbackValue;
