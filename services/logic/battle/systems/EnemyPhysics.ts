...
import { EliteManagerConfig } from "../special/EliteManagerConfig";
...
export const EnemyPhysics = {
    ...
    checkSoftCollisions: (engine: IGameEngine, e: any, grid: SpatialHashGrid) => {
        // IMPORTANT: If this enemy is latched, it CANNOT be pushed. It is part of the player now.
        if (e.captureState === 'latched') return;

        const neighbors = grid.query(e.x, e.y);

        for (const other of neighbors) {
            if (e === other) continue;
            if (e.linkedById || other.linkedById) continue; 
            if (e.captureState || other.captureState) continue; 
    
            // --- DEVOURER EXCEPTION (ALLOW OVERLAP) ---
            // If one is Devourer and other is prey (not boss/elite), allow overlap so eating can happen
            // The devourer logic will pull them into center.
            const isDevourerE = e.config.behavior === 'devourer';
            const isDevourerOther = other.config.behavior === 'devourer';
            
            if (isDevourerE || isDevourerOther) {
                const devourer = isDevourerE ? e : other;
                const prey = isDevourerE ? other : e;
                
                // Allow overlap if prey is edible (common/rare/minion)
                if (prey.config.behavior !== 'boss' && prey.config.behavior !== 'devourer' && prey.config.tier !== 'epic' && prey.config.tier !== 'boss') {
                    // Do NOT apply repulsion force here.
                    // This allows the AggressiveSkills logic to pull the prey into the center point (dist < 10)
                    continue; 
                }
            }
            ...
            if (odist < e.radius + other.radius) {
              if (e.config.behavior === 'boss' && other.config.behavior === 'minion') {
                 spawnFloatingText(engine, e.x, e.y - e.radius - 20, "压榨!", "#ef4444", 'chat');
                 e.hp = Math.min(e.maxHp, e.hp + 200);
                 spawnFloatingText(engine, e.x, e.y, "+200", "#22c55e");
                 other.hp = 0; 
                 engine.state.enemies = engine.state.enemies.filter((en: any) => en !== other);
                 return;
              }
              const push = 0.2; 
              e.vx += odx / odist * push;
